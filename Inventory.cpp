...
pair<int, bool> getOrCreateSupplierID(sql::Connection* con) {
    string supplierName, contactName, contactNumber, email, address;
    cout << "Enter Supplier Name: "; cin >> ws; getline(cin, supplierName);

    sql::PreparedStatement* stmt = con->prepareStatement("SELECT SupplierID, ContactName, ContactNumber, Email, Address FROM Suppliers WHERE Name = ?");
    stmt->setString(1, supplierName);
    sql::ResultSet* res = stmt->executeQuery();

    if (res->next()) {
        int supplierID = res->getInt("SupplierID");
        string dbContactName = res->getString("ContactName");
        string dbContactNumber = res->getString("ContactNumber");
        ...
        if (dbContactName == contactName && dbContactNumber == contactNumber && dbEmail == email && dbAddress == address) {
            delete res;
            delete stmt;
            return make_pair(supplierID, false);
        }
    }
    delete res;
    delete stmt;
    ...
    delete res;
    delete stmt;
    return make_pair(newSupplierID, true);
}
...
void addProduct(sql::Connection* con) {
    try {
        ...
        auto supplierResult = getOrCreateSupplierID(con);
        int supplierID = supplierResult.first;
    ...
    try {
        ...
        while (res->next()) {
            cout << std::left << std::setw(10) << res->getInt("ProductID") << " | "
                ...
                << std::setw(15) << res->getDouble("CostPrice") << " | "
